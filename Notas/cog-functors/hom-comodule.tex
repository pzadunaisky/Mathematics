...
are over $\k$ unless otherwise stated. Hiven a coalgebra $H$ we denote by $\Mod^H$ the
=======
...
>>>>>>> fe01f5c2424d7badfb9128ab4741dd41320e69dd
category of right $H$-comodules and write $\Hom^H$ for $\Hom_{\Mod^H}$.

\section{Hom comodules}
\paragraph
<<<<<<< HEAD
Let $H$ be a Hopf algebra and let $M, N$ be $H$-comodules. We define the $H$-comodule-hom
space $\HOM^{H}(M, N)$ as the pull-back
\begin{align*}
	\xymatrix{
		\Hom_\k(M, N) \ar[r]^-{\rho^{N}_*} & \Hom_\k(M, N \ot H) \\
		\HOM^{H}(M, N) \ar@{-->}[u]^-\alpha \ar@{-->}[r]^-\rho & \Hom_\k(M, N) \ot H 
		\ar[u]_-{\Gamma}
	}
=======
...
<<<<<<< HEAD
It is also true that $\Gamma$ is injective. Let $T: \Hom(M,N \ot H) \to \Hom_\k(M,N \ot
H)$ be defined by $T(F)(m) = f(m_0) \cdot 1 \ot S(m_1)$. Thus we get a commutative diagram
\begin{align*}
	\xymatrix{
		\Hom_\k(M, N \ot H) \ar[dr]^-T \\
		\Hom_\k(M, N) \ot H \ar[u]^\Gamma \ar[r]^-i & \Hom_\k(M,N \ot H)
	}
=======
...
<<<<<<< HEAD
Hence $\HOM^H(M,N) \subset \Hom_\k(M,N)$ consists of those functions $f: M \to N$ for
which there exist $f_1, \ldots, f_r \in \Hom_\k(M,N)$ and $g_1, \ldots, g_r
\in H$ such that for every $m \in M$ we get $f(m)_0 \ot f(m)_1 = \sum_{i = 1}^r f_i(m_0)
\ot g_i m_i$. Notice that the submodule of coinvariants of $\HOM^H(M,N)$ is $\Hom^H(M,N)$.
=======
...
\paragraph
For example, suppose $H$ is the group algebra of $\hat H$, in which case $M, N$ are
$\hat H$-graded modules. Then $\HOM^H(M,N)$ consists of those morphisms which are
finite sums of homogeneous morphisms of arbitrary degree, i.e. $\HOM^H(M,N) =
\bigoplus_{g \in \hat H}\Hom^H(M,N[g])$. It is a well-known result that if $M$ is finite
dimensional then $\HOM^H(M,N) = \Hom_\k(M,N)$.

On the other hand, suppose $\k$ has characteristic zero and let $H$ be the enveloping
algebra of the one dimensional Lie algebra with basis $X$. A $H$-comodule is a vector
space $M$ with a locally nilpotent endomorphism $t: M \to M$ such that $\rho^M(m) = \sum_n
\frac{t^n(m)}{n!} \ot X^n$. Suppose $M, N$ are $H$-comodules with structure maps given by
endomorphisms $t, u$. Take $F \in \Hom_\k(M,N)$, and set
\[
	F_n = \sum_{i + j = n} \frac{(-1)^{i}}{i!j!} u^{j} \circ F \circ t^i.
\]
Then $F \in \HOM^H(M, N)$ if and only if $F_n = 0$ for $n \gg 0$. In that case $\rho(F) =
...
M$. Thus if $M$ is finite dimensional $\HOM^H(M,N) = \Hom_\k(M,N)$

\paragraph
\label{hom-H-comodule}
The $\k$-vector space $\HOM^{H}(M,N)$ is a $H$-comodule. Indeed, consider the following
diagram
\begin{align*}
	\xymatrix{
		\Hom_\k(M,N) \ar[r]^{\rho^{N}_*}
		& \Hom_\k(M,N \ot H) \ar[r]^-{(\rho^{N} \ot 1)_*}
			& \Hom_\k(M,N \ot H \ot H)\\
		\HOM^H(M,N) \ar[r]^-\rho \ar[u]^-\alpha
			&\Hom_\k(M,N) \ot H \ar[r]^-{\rho^{N}_* \ot 1} \ar[u]^-\Gamma \ar[dr]^-{1
				\ot \Delta}
			& \Hom_\k(M, N \ot H) \ot H \ar[u]_-\Gamma \\
		{} 
		&\HOM^{H}(M,N) \ot H \ar[u]^-{\alpha \ot 1} \ar[r]^-{\rho \ot 1}
			& \Hom_\k(M,N) \ot H \ot H \ar[u]_-{\Gamma \ot 1}\\
	}
\end{align*} 
...
image of $\rho$ is contained in $\HOM^H(M,N) \ot H$ it is enough to show that $T = U$.
...
\begin{Proposition}
	Let $M,N$ be objects of $\Mod^H$. If $M$ is finitely generated then $\HOM^H(M,N) =
	\Hom_\k(M,N)$.
\end{Proposition}
\begin{proof}
	Fix $f \in \Hom_\k(M,N)$. Choose a basis $\B$ of $H$ and for each $m \in M$ and $g \in
	...
	\end{align*}
	which shows that $f \in \HOM^H(M,N)$.
\end{proof}
...
\paragraph
Let $H, H$ be Hopf-algebras and let $\phi: H \to H$ be a Hopf-algebra morphism. We see $H$
as a left $H$-comodule with structure morphism $\rho^H = \phi \ot 1 \circ \Delta_H: H \to
H \ot H$. Let $L = {}^{\co H} H \subset H$ be the subalgebra of coinvariants. Suppose $L
\subset H$ is cleft, i.e. there exists a convolution invertible morphism of left
$H$-comodules $\gamma: H \to H$. The fact that $\gamma$ is a comodule morphism implies
that 
\begin{align}
	\label{gamma}
	\phi(\gamma(h)_1) \ot \gamma(h)_2 = h_1 \ot \gamma(h_2)
\end{align}
Let $\gamma^{-1}$ be the convolution-inverse of $\gamma$. Since $\rho^H$ is an algebra
morphism, $\rho^H \circ \gamma$ is convolution invertible with inverse given by $\rho^H
\circ \gamma^{-1}$. On the other hand, setting $\sigma = S \ot 1 \circ \tau \circ
\Delta_H$ where $\tau$ is the flip, we see that
\begin{align*}
	\rho^H \circ \gamma * \sigma (h) 
		&= \rho^H \circ \gamma(h_1)\sigma(h_2) \\
...
\begin{align}
	\label{gamma-inverse}
	\rho^H \circ \gamma^{-1}(h) =  S(h_2) \ot \gamma^{-1}(h_1).
\end{align}

%\begin{Lemma}
%	For every $h \in H$ and every $l \in L$, the element $\gamma^{-1}(h_1) \gamma(h_2)_1
%	l_1 \ot \gamma(h_2)_2 l_2$ belongs to $L \ot H$.
%\end{Lemma}
%\begin{proof}
...
%		\Delta_H(\gamma(h_2)l)
%	\]
%	Using	\ref{gamma-inverse} we get $\rho^H \ot 1 (\gamma^{-1}(h_1)) = S(h_2) \ot
%	\gamma^{-1}(h_1) \ot 1$. On the other hand, using \ref{gamma}
%	\begin{align*}
%		\rho^H\ot 1( \Delta_H(\gamma(h_3)l)) 
%		&= \phi(\gamma(h_3)_1) \phi(l_1) \ot \gamma(h_3)_2 l_2 \ot \gamma(h_3)_3 l_3 \\
%		&= 1 \ot \Delta_H(\phi(\gamma(h_3)_1) \phi(l_1) \ot \gamma(h_3)_2 l_2)\\
%		&= 1 \ot \Delta_H(h_3 \ot \gamma(h_4)l) = h_3 \ot \gamma(h_4)_1 l_1 \ot
%		\gamma(h_4)_2 l_2.
%	\end{align*}
%	Hence
%	\begin{align*}
%		\rho^H \ot 1 &(\gamma^{-1}(h_1) \gamma(h_2)_1 l_1 \ot \gamma(h_2)_2l_2 )) \\
%		&= \rho^H \ot 1(\gamma^{-1}(h_1) \ot 1) \cdot \rho^H \ot 1(\Delta_H(\gamma(h_2)l)) \\
...
\paragraph
Hiven a $H$-comodule $M$, we define a map $\Gamma: \hom_\k(L,M) \ot H \to \hom_\k(L, M \ot
H)$ by
\begin{align*}
	\Gamma(f \ot h)(l) = f \ot 1 ((\gamma^{-1}(h_1)\gamma(h_2)_1 \ot \gamma(h_2)_2) \cdot
	(l_1 \ot l_2)).
\end{align*}
By the previous lemma, the argument of the function lies in $L \ot H$ so this is
well-defined. We define $\phi_*(M)$ to be the pull-back of the following diagram
\begin{align*}
	\xymatrix{
		\hom_\k(L,M) \ar[r]^-{\rho^M_*} & \hom_\k(L, M \ot H) \\
...
Recall that a \emph{cross-section} of $\phi: H \to H$ is a co-algebra morphism $\gamma: H
\to H$ such that $\phi \circ \gamma = \Id_H$.
\begin{Proposition*}
	Suppose $\gamma: H \to H$ is a cross-section of $\phi$. Then for every $H$-comodule $M$
...
\begin{proof}
	...
	\begin{align*}
	\xymatrix{	
		\hom_\k(L,M \ot H) \ar[r]^-T 
			& \hom_\k(L, M \ot H) \ar[d]^-{(1\ot \phi)_*} \\
...
$\gamma: H \to H$ is a cross-section of $\phi$, then it is convolution invertible with
...
\gamma^{-1}(h_1)$.
\begin{Theorem*}
	Suppose $\phi: H \to H$ has a cross-section $\gamma$. Then the functor $\phi_*: \Mod^H
	\to \Mod^H$ is left adjoint to $\phi^*: \Mod^H \to \Mod^H$.
\end{Theorem*}
\begin{proof}
	We wish to find for every $N$ in $\Mod^H$ and every $M$ in $\Mod^H$ isomorphisms
	\begin{align*}
	\xymatrix{
		\hom^H(N, \phi_*(M)) \ar@/^18pt/[rr]^-{\alpha} 
			&{}
			& \hom^H(\phi^*(N), M) \ar@/^15pt/[ll]^-\beta.
	}
	\end{align*}
	...
	\]
	We must prove that $\alpha(f)$ is a $H$-module morphism, that is $\rho^M \circ
...
\paragraph
\begin{enumerate}
	\item OK for now. It should work with $\gamma$ an $H$-comodule section, i.e. $H$ an
		$H$-cleft extension.
\end{enumerate}
