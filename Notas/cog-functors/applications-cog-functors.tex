...
to $f_g$ as the homogeneous component of degree $g$ of $f$. The \emph{support}
of a $G$-graded vector space $V$ is the set $\supp V = \{g \in G \mid V_g 
\neq 0\}$.
...
Given an object $V$ in $\Vect^G \k$ and $g \in G$, we denote by $V[g]$ the
$G$-graded vector space whose homogeneous component of degree $g'$ is
$V[g]_{g'} = V_{g'g}$.

The category $\Vect^G V$ has arbitrary direct sums and products. The direct sum
...
M_{gg'}$. Notice also that if $M$ is a $G$-graded $A$-module then the 
$G$-graded vector space $M[g]$ is also a $G$-graded $A$-module, with the same 
...
$\injdim_A^G 
M$ its projective and injective dimension, respectively. Given two $G$-graded
$A$-modules $M, N$ we denote by $\Hom^G_A(M,N)$ the vector space of all 
$G$-homogeneous $A$-linear morphisms from $M$ to $N$. Since $\Mod^G A$ has 
enough injectives, we can define for each $i \geq 0$ the $i$-th right derived 
functor of $\Hom^G_A$, which we denote by $\R^i\Hom^G_A$. 
...
algebra. The functor $\phi^*$ is right adjoint to $\phi_!$ and left adjoint to 
 $\phi_*$. 
\end{Proposition}
Let $M$ be a $G$-graded $A$-module. There are natural isomorphisms
\begin{align*}
\phi^*\phi_!(M) 
  &\cong \bigoplus_{l \in L} M[l]
  &\phi^*\phi_*(M) 
  &\cong \prod_{l \in L} M[l]
\end{align*}
...
only if it is projective as $A$-module, i.e. $\phi_!(M)$ is projective. The
following proposition is a refined version of this result.
...
\paragraph
\label{phi-finite}
As is well known, an injective object $I$ in $\Mod^G A$ need not be injective
as $A$-module. Our next objective is to give a bound to the injective dimension
of $\phi_!(I)$. 

We begin with the following observation. The natural inclusion of the direct 
sum of a family of vector spaces into its product gives rise to a natural 
transformation $\eta: \phi_! \to \phi_*$. Notice that $\eta(M): \phi_!(M) \to
\phi_*(M)$ is an isomorphism if and only if for each $h \in H$ the set 
$\supp M \cap \phi^{-1}(h)$ is finite. 

\begin{Definition*}
An object $M$ of $\Mod^G A$ is called \emph{$\phi$-finite} if for each $h \in 
H$ the set $\supp M \cap \phi^{-1}(h)$ is finite. 
\end{Definition*}
The following lemma is an immediate consequence of Proposition \ref{P:adjoint}.
\begin{Lemma*}
If $M$ is $\phi$-finite then $\injdim^G_A M = \injdim_A^H \phi_!(M)$. 
\end{Lemma*}
In particular, if $A$ is $\phi$-finite then every finitely generated $G$-graded
$A$-module is $\phi$-finite, and hence $\phi_!$ preserves the injective 
dimension of all finitely generated $G$-graded $A$-modules.
...
\label{P:resolution}
\begin{Definition*}
Let $N$ be an $H$-graded $A$-module. We say that $N$ is \emph{stably 
$G$-graded} if there exist an $H$-graded $A$-module $N'$ and a $G$-graded 
...
\label{L:acyclic}
\begin{Lemma}
Assume $A$ is left $G$-noetherian. Let $I$ be an injective object of 
$\Mod^G A$. If $N$ is stably $G$-graded then $\R^i\Hom_A^H(N, \phi_!(I)) = 0$
for all $i > 0$.
\end{Lemma}
\begin{proof}
...
$A$ is left $G$-noetherian implies that $\bigoplus_{l \in L} I[l]$ is 
...
\paragraph
In this section we denote by $A$ an $\NN^r$-graded algebra and use
the usual notation $A^e = A \ot A^\op$; under this hypothesis $A$ is 
noetherian if and only if it is $\ZZ^r$-noetherian. We will say that $A$ is 
connected if $A_0 = \k$. We denote by $\D^{\ZZ^r}(A), \D^{\ZZ^r}(A^\op)$ and 
$\D^{\ZZ^r}(A^e)$ the derived 
categories of $\Mod^{\ZZ^r} A$, $\Mod^{\ZZ^r} A^{\op}$ and $\Mod^{\ZZ^r} A^e$,
respectively. We write $\R\HOM^{\ZZ^r}_A$ for the right derived functor of the 
internal $\HOM$ functor of the category of complexes of $\ZZ^r$-graded 
bimodules, as defined in \cite{Yek-dc}*{section 2}. Recall that an injective 
resolution of a left bounded complex $R^\bullet$ is an isomorphism $R^\bullet 
\to I^\bullet$ in $\D^{\ZZ^r}(A)$, where $I^\bullet$ is a left bounded complex
of injective $\ZZ^r$-graded modules, and that the injective dimension of 
$R^\bullet$ is the minimal length of an injective resolution. We write 
$\injdim^{\ZZ^r}_A R^\bullet$ for the injective dimension of $R^\bullet$.

\paragraph
\label{dc}
The following definition is adapted from \cite{Yek-dc}*{Definition 3.3}
\begin{Definition*}
Let $A$ be an $\NN^r$-graded connected algebra.
A \emph{$\ZZ^r$-graded dualizing complex} is a bounded complex $R^\bullet$ in 
$\D^{\ZZ^r}(A^e)$ such that:
\begin{enumerate}
...
\item the natural maps $A^\op \to \mathcal R\HOM_{A} (R^\bullet, R^\bullet)$
  and $A \to \mathcal R\HOM_{A^\op}(R^\bullet, R^\bullet)$ are isomorphisms
  in $\D^{\ZZ^r}(A^e)$.
\end{enumerate}
\end{Definition*}
In particular a \emph{dualizing complex in the ungraded sense} is an object in 
...
\Mod^{\ZZ^r} A \to \Mod A$ is exact, it induces an exact functor $\O: 
\D^{\ZZ^r}(A) \to \D(A)$. The other change of grading functors also induce 
functors between the corresponding derived categories. We will need the 
following lemma, which is a derived version of Theorem 
\ref{T:main-theorem}. 
\begin{Lemma*}
\label{derived-inj-dim}
Assume $A$ is a connected $\NN^r$-graded left noetherian algebra. If 
$R^\bullet$ is an object of $\D^{\ZZ^r}(A)$ then
\[
...
\end{Lemma*}
\begin{proof}
Both inequalities are proved as in Theorem \ref{T:main-theorem}. The syzygy 
argument for the second inequality is adapted by taking a minimal injective 
resolution of $R^\bullet$ and, assuming it starts in homological degree $0$, 
using the exact sequence 
\[
  0 \to I^{\geq 1} \to I^\bullet \to I^0 \to 0.
\]
\end{proof}
