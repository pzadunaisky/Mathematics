...
\textbf{Notation:} For each $n,m \in \NN$ with we set $\ll m,n \rr = \{i \in 
\NN \mid m \leq i \leq n\}$. Given two elements $a, b$ of a $\CC$-algebra $A$, 
...
its projection on $\Lambda_m/J$ is basis of this vector space. Thus if the 
...
The projection of $\B'$ to the quotient $\Lambda_m/J$ is basis.
...
1 \leq i < j \leq k \leq n, a \in \ZZ\}$. The algebra $A$ contains all the 
...
that $A$ is stable by the action of $S_m$. Consider the action of $\ZZ^N$ over 
$\CC^N$, given by $v^z = v+z$. This induces an action on $A$, given by 
...
a tableaux $z$. Set $V_A = A \ot_\CC V_\CC$; we write $fT(z)$ for the 
elementary tensor $f \ot T(z)$ where $f \in A$ and $z \in \ZZ_0^N$. The 
...
\end{align*}
where $\epsilon$ runs over the stabilizer of $v$. In particular if $v$ is 
sufficiently noncritical (say all entries differ by more than $2$) then this 
...
\end{align*}
Using the inner product describe above
\begin{align*}
