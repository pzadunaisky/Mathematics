...
Recall that a semigroup $S$ is said to be well-ordered if there exists a 
well-order $<$ on $S$ compatible with the additive structure, i.e. such that 
for all $s,s',s'' \in S$ the inequality $s<s'$ implies $s+s'' < s'+s''$. 
...
An \emph{$(S,<)$-basis} for $A$ is an ordered basis $\{b_s \mid s \in S\}$ 
consisting of homogeneous elements, such that for all $s,s',s'' \in S$ with 
$s'' < s+s'$ there exist $c_{s,s'} \in \k^\times$ and $c_{s,s'}^{s''} \in \k$ 
such that
\[
...
\paragraph
\label{S-order-bases}
Let $S$ be a semigroup. Assume $<$ is a well-order on $S$ compatible with its 
additive structure, and let $A$ be an algebra. An $(S,<)$-filtration on $A$ is 
a collection of vector spaces $\F = \{F_sA \mid s \in S\}$, such that $F_sA 
\cdot F_{s'} A \subset F_{s+s'}A$ for all $s,s' \in S$, and such that $F_sA 
\subset F_{s'}A$ whenever $s < s'$. The standard notions related to 
$\NN$-filtrations translate easily to the context of $(S,<)$-filtrations. In 
particular, one can define the associated graded algebra $\gr_\F A$ by setting 
$F_{<s}A = \sum_{t<s} F_tA$ and taking
\[
...
write $P = \{\varpi_\alpha \mid \alpha \in \Pi\}$ for the corresponding set of 
fundamental weights. We denote by $\Lambda$ the weight lattice $\sum_{\alpha 
\in \Pi} \ZZ \varpi_\alpha$ and by $\Lambda^+$ the set of dominant weights 
$\sum_{\alpha \in \Pi} \NN \varpi_\alpha$. 

Let $W$ be the Weyl group of $\g$, and $s_\alpha \in W$ the reflection 
corresponding to the $\alpha \in \Pi$. Given an element $w \in W$ we denote 
...
spaces $\bigoplus_{\mu \in \Lambda} V_q(\lambda)_\mu$; the dimensions of the 
weight spaces are the same as in the classical case, so the Weyl character 
formulas holds for these representations, see \cite{Jan}*{5.15}. 
...
\sum_{i+j=r}^r v_i^{ji} F^{(i)}_\alpha \ot F^{(j)}_\alpha K_\alpha^{-j}$. 
...
of the form $d_{b,b'}^{b''} (G(b) \ot G(b') K_{\wt(b)})$, perhaps with 
...
longest word of $W$. For each $a \in \NN^N$ we write $F^{(a)} = 
F_{\alpha_1}^{(a_1)} \cdots F_{\alpha_N}^{(a_N)}$ and $\tilde F^{a} = 
\tilde F_{\alpha_1}^{a_1} \cdots \tilde F_{\alpha_N}^{a_N}$. If $a$ lies in 
the image of $\Lambda_{\tilde w_0}$ the we write $b_a = 
\Lambda_{\tilde w_0}^{-1}(a)$. Let $\nu \in \ZZ \Phi$ with $\nu \geq 0$.
\begin{enumerate}[label=(\alph*)]
\item If $b_a \in \B(\infty)_{-\nu}$ then for each $a' \in 
...
\item Let $\alpha \in \Pi$ and $r > 0$. Let $b \in \B(\infty)_{-\nu}$. Then
for each $b' \in \B(\infty)_{-\nu - r\alpha}$ there exists $x_{b'} \in \A$ 
such that
\begin{align*}
F^{(r)}_\alpha G(b) 
  &= G(\tilde F_{\alpha}^r b) + \sum_{e_{\alpha}(b') > r} x_{b'} G(b')
\end{align*}

\item If $a \in \NN^N$ and $F^{(a)} \in U^-_{-\nu}$, then for each $a'\in 
\Lambda_{\tilde w_0} (\B(\infty)_{-\nu})$ with $a' >_{\lex} a$ there exists 
$z_{a,a'} \in \A$ such that
\begin{align*}
F^{(a)}
  &= G(\tilde F^a 1) + \sum_{a' >_\lex a} z_{a,a'} G(b_{a'}).
\end{align*}
Furthermore $\Lambda_{\tilde w_0}(\tilde F^a 1) >_{\lex} a$.
\end{enumerate}
...
$b_{a'} \in \B(\infty)_{-\nu}$. This completes the proof of the first item.

By \cite{Jan}*{Lemma 11.3} we get that $F_\alpha^{(r)} G(b) \equiv 
F_\alpha^{(r)} G(b) \mod \tilde F_\alpha^{r+1} U^-_\A$. Now as shown in the 
proof of \cite{Jan}*{Lemma 11.12 (b), page 249}, $\tilde F_\alpha^r G(b) 
\equiv G(\tilde F_\alpha^r b) \mod \tilde F_\alpha^{r+1}U^-_\A$; the same 
lemma states that $F_\alpha^{r+1}U^-_\A$ is generated over $\A$ by the $G(b')$
with $e_{\alpha}(b) \geq r+1$, so the second item follows.

For the last item, let $j = \max\{i \mid a_i \neq 0\}$, so $F^{(a)} = 
F_{\alpha_j}^{(a_j)} \cdots F_{\alpha_N}^{(a_N)}$. Induction using the previous
item shows that
\begin{align*}
F^{(a)} 
  &= G(\tilde F^a 1) + \sum_{e_{\alpha_j}(b') > a_j} z_{b,b'} G(b').
\end{align*}
Notice that the condition $e_{\alpha_j}(b') > a_j$ automatically implies that
$\Lambda_{\tilde w_0}(b') >_{\lex} a$. Let $b$ be the element of the crystal
basis corresponding to $\tilde F^a 1$, and let $s = \Lambda_{w_0}(b)$. If 
$s_i \neq 0$ for some $i < j$ then $s > a$ and we are finished. Now if this
is not the case, then since $\tilde E_{\alpha_j}^{a_j} b = \tilde 
E_{\alpha_j}^{a_j} \tilde F^a 1 \neq 0$ implies that $s_j \geq a_j$. If $s_j
> a_j$ then $s > a$, otherwise we may repeat the reasoning with $\tilde 
E_{\alpha_j}^{a_j} b$ to show that $s_{j+1} \geq a_{j+1}$, etc. Since $a$
is not in the image of $\Lambda_{\tilde w_0}$, we must eventually reach a
strict inequality which shows that $s > a$.
\end{proof}
...
F^{(t)} \ot F^{(u)}K_{\wt(F^{(t)})}$, with $z(t,u) \in \ZZ$. Now the first 
item of the previous lemma implies that
\begin{align*}
\Delta(G(b_a))
  ...
  + \sum_{a' >_{\lex} a} \sum_{t+u = a'} y_{a,a'} v^{z(t,u)} F^{(t)}
    \ot F^{(u)}K_{\wt(F^{(t)})}
\end{align*}
and using the third item we can restrict the sum to the $s,t$ in the
image of $\Lambda_{\tilde w_0}$, so the element in the last display
equals
\begin{align*}
