...
\paragraph
We work with $\k = \QQ(v)$ and $q = v$. We denote by $U^+$ and $U^-$ the 
algebras $U^+_v(\g)$ and $U^-_v(\g)$, respectively. Let $\A = \ZZ[v, v^{-1}] 
\subset \QQ(v)$. The algebra $U$ has an $\A$-form, defined in 
\cite{Jan}*{section 11.1}, which we denote by $U_\A$. This is a subring of 
$U$. The algebras $U^+$ and $U^-$ also have $\A$-forms which we denote by 
$U_\A^+$ and $U_\A^-$ respectively, and all these $\A$-forms are compatible 
with the weight decomposition of $U$. By construction $U \cong \QQ(v) \ot_\A 
U_\A$, and analogous results hold for $U_\A^+$ and $U_\A^-$. Furthermore the 
highest weight modules $V_v(\lambda)$ have corresponding $\A$-forms which we 
denote by $V_\A(\lambda)$. All these $\A$-forms are compatible with the weight 
decompositions of the original objects, see \cite{Jan}*{chapters 9,10}. 
Now let $\lambda$ be a dominant weight and fix $w \in W$. The $\A$-form of the 
Demazure module $V_q(\lambda)_{w}$ is defined as $V_\A(\lambda)_{w} = U^+ 
...
The operators $\tilde E_i, \tilde F_i$ of \cite{Jan}*{10.2} induce operators 
$\tilde E_i, \tilde F_i: \B \to \B \cup \{0\}$. The operators $\tilde E_i$ are 
locally nilpotent, so it makes sense to set $e_{\alpha_i} = e_i(b) = \max 
\{k \in \NN \mid \tilde E_i(b) \neq 0\}$ for each $b \in \B$. Write 
$\overline E_i(b)$ for $\tilde E_i^{e_i(b)}(b)$.
...
the equations $a_{N+i} = 0$ for all $i$ such that $\varpi_i \in I$, and hence 
...
$a_i = 0$ for all $\ell(w) < i \leq N$, and hence it is also a normal affine 
semigroup.
...
semigroups. All that is left is to check that they have the multiplicative 
property of $(S,<)$-bases, where $<$ is the lexicographic order of $\NN^N 
\times \NN^n$.

Let $\lambda, \lambda' \in \Lambda^+$ while $b \in \B(\lambda), b' \in
...
b'_{\lambda'b}$ is by definition the matrix coefficient corresponding to the 
functional $b_\lambda^* \ot b'_{\lambda'}$ and the vector $v_\lambda \ot 
...
$b_\lambda^* \ot b'_{\lambda'}$ naturally induces an element in $C^+(\lambda 
...
\sum_{j=0}^r q_i^{j(r-j)} F^{(j)}_i \ot F^{(r-j)}_i K_i^{-j}$. Since the 
...
these terms there is one of the form $d_{b,b'}^{b''} (b \ot b' K_\lambda)$, 
perhaps with $d_{b,b'}^{b''} = 0$, and from the reasoning above we get that 
$c_{b,b'}^{b''} = q^{(\lambda, \lambda')} d_{b,b'}^{b''}$. Notice that unlike 
before, the element $d_{b,b'}^{b''}$ is independent of $\lambda$ and 
$\lambda'$.
...
\end{align*} 
\end{Proposition*} 
We give a sketch of the proof in the following paragraphs.
...
Now in order to prove this result about the dual of the canonical basis, we 
...
in \cite{Jan}*{6.14}. Set $L_i^{(n)} = \frac{1}{[n]!} L_i^n$. Fixing $b \in 
\B(\infty)$ we get that
\begin{align*}
1 = (G(b), G(b)^*) = ()
\end{align*}
