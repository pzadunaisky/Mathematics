...
$\m$. Clearly $A/\m \cong \k$ as vector spaces, and we will use this 
isomorphism to give $\k$ the structure of an $A$-bimodule.
...
use an argument due to P. Caldero to show that these algebras have 
...
of Littelman's string parametrization of this basis.

While Caldero's argument is based on a diferent definition of the quantized 
enveloping algebra, and he works over the trascendental extension $\CC(q)$ 
while we work over $\QQ(v)$, his argument works \emph{mutatis mutandis}. We 
believe it is worthwhile to retrace it in some detail, as the ingredients 
mentioned above are quite technical. 

\subsection*{Quantum flag and Schubert varieties}

\paragraph
\label{lie-algebra-notation}
Let $\g$ be a semisimple complex Lie algebra. We denote by $\Phi = \{\alpha_1, 
\ldots, \alpha_n\}$ the set of simple roots of $\g$, by $\ZZ\Phi$ its root 
lattice, by $P = \{\varpi_1, \ldots, \varpi_n\}$ the set of fundamental 
weights, and by $\Lambda$ its weight lattice. Set $\Lambda^+ = \sum_i 
\NN \varpi_i$, the set of dominant weights. 

Let $W$ be the Weyl group of $\g$, and $s_i \in W$ the reflection 
corresponding to the $i$-th simple root. Given an element $w \in W$ we denote 
...
generators $s_i$ that equals $w$ in $W$. The decomposition is \emph{reduced}
if it is of minimal length, i.e. its length equals $\ell(w)$. We denote by 
$(-,-)$ the standard $W$-invariant pairing between $\ZZ\Phi$ and $\Lambda^+$,
and write $\langle \lambda, \alpha\rangle = 
\frac{2(\lambda,\alpha)}{(\alpha,\alpha)}$ for all $\lambda \in \Lambda^+$ and
$\alpha \in \Phi$, so if $\lambda = \sum_i r_i \varpi_i$ then $\langle \lambda,
\varpi_k \rangle = r_k$.


\paragraph
\label{quantized-enveloping-algebra}
Fix $q \in \k^\times$. Let $U_q(\g)$ be the quantum enveloping 
algebra of $\g$ as defined in \cite{Jan}*{Definition 4.3}. Let $U_q^+(\g), 
U_q^-(\g)$ be the positive and negative parts of $U_q(\g)$ as defined in 
\cite{Jan}*{4.4}. If $q$ is not a root of unity then by \cite{Jan}*{chapter 5} 
for each $\lambda \in \Lambda^+$ there is an irreducible highest-weight 
representation of $U_q(\g)$ of type $\mathbf{1}$, which we denote by 
$V_q(\lambda)$. Each $V_q(\lambda)$ decomposes as the direct sum of weight 
spaces $\bigoplus_{\mu \in L(\lambda)} V_q(\lambda)_\mu$, where $L(\lambda)$ 
is a finite subset of $\Lambda$; the dimensions of the weight spaces are the 
same as in the classical case, so the Weyl character formulas holds for these 
representations, see \cite{Jan}*{subsection 5.15}. 

\paragraph
Let $G$ be the simply connected Lie group corresponding to $\g$. Since 
$U_q(\g)$ is a Hopf algebra, its dual $U_q(\g)^*$ is an algebra 
...
Quantum flag varieties were introduced by Soibelman in \cite{S} and by 
Lakshmibai and Reshetikhin in \cite{qLR}; we review their definition. 
...
generated by the reflections $s_i$ with $\varpi_i \in I$, and for each class 
in $W/W_I$ pick a representative of minimal length. We denote by $W^I$ the set 
of these representatives. Since the Weyl character formula holds, for each 
$w \in W$ and each $\lambda \in \mathcal J(I)$ the vector space 
$V_q(\lambda)_{w\lambda}$ has dimension $1$. The \emph{Demazure module} 
...
generalized flag variety. To this datum we associate the $\Lambda^+$-graded 
...
$I = \emptyset$ corresponds to the full flag variety, while $I = P \setminus 
\{\varpi_k\}$ corresponds to the $(k,n)$-grassmannian.
...
We now review Caldero's argument on the existence of an $(S,<)$-basis of 
$\O_q[G/B]$, and its natural extension to arbitrary partial flag and Schubert 
varieties. Since he was interested in classical flag varieties, Caldero works 
with a large base field $\CC(q)$ that allows him to specialize at $q=1$ and 
still get algebraic varieties over the complex numbers. His arguments go 
through almost word by word in our context, but we believe it is worthwhile to 
retrace it in some detail.

\paragraph
We work with $\k = \QQ(v)$ and $q = v$, and set $U=U_v(\G)$. We denote by 
$U^+$ and $U^-$ the algebras $U^+_v(\g)$ and $U^-_v(\g)$, respectively. Let 
$\A = \ZZ[v, v^{-1}] \subset \QQ(v)$. The algebra $U$ has an $\A$-form, 
defined in \cite{Jan}*{section 11.1}, which we denote by $U_\A$. This is a 
subring of $U$. The algebras $U^+$ and $U^-$ also have $\A$-forms which we 
denote by $U_\A^+$ and $U_\A^-$ respectively, and all these $\A$-forms are 
compatible with the weight decomposition of $U$. By construction $U \cong 
\QQ(v) \ot_\A U_\A$, and analogous results hold for $U_\A^+$ and $U_\A^-$. 
Furthermore the highest weight modules $V_v(\lambda)$ have corresponding 
$\A$-forms which we denote by $V_\A(\lambda)$; these are obtained by fixing a 
highest weight vector $v_\lambda$ and looking at the $\A$-lattice $U^-_\A 
v_\lambda \subset V_v(\lambda)$. These $\A$-forms are compatible with the 
weight decompositions of the original objects, see \cite{Jan}*{chapters 9,10}. 
Now let $\lambda$ be a dominant weight and fix $w \in W$. The $\A$-form of the 
Demazure module $V_q(\lambda)_{w}$ is defined as $V_\A(\lambda)_{w} = U_\A^+ 
...
at $0$. For each $1 \leq i \leq n$ define the operators $\tilde F_i = \tilde 
F_{\alpha_i}: U^- \to U^-$ as in \cite{Jan}*{10.2}, and let $\L(\infty)$ be 
the $A_0$-lattice generated by all elements of the form $\tilde F_{i_1} \tilde 
F_{i_2} \cdots \tilde F_{i_r}(1)$; by definition these are weight elements, so 
setting $\L(\infty)_{-\nu} = \L(\infty) \cap U^-_{-\nu}$ for each $\nu \in \ZZ
\Phi$ with $\nu \geq 0$, we get $\L(\infty) = \bigoplus_{\nu \geq 0} 
\L(\infty)_{-\nu}$, and furthermore each $\L(\infty)_{-\nu}$ is a finitely 
generated $A_0$-module that generates $U^-_\nu$ over $\QQ(v)$. 
...
running over all dominant weights, we obtain a basis of $\O_v[G/B]$
\[
...
partial flag variety $\O_v[G/P_I]$ defined in \ref{q-partial-flag-varieties} 
by taking
\[
...
defined in \ref{q-Schubert-varieties} is spanned over $\QQ(v)$ by all elements 
of the form $b_\lambda$ with $b \in \B(\lambda) \setminus \B_w(\infty)$. Thus 
we can obtain a basis for the quantum Schubert variety $\O_v[G/P_I]_w$ by 
taking the image of
\[
  \schubertbasis_I(w) 
...
The operators $\tilde E_i, \tilde F_i$ induce operators $\tilde E_i, 
\tilde F_i: \B(\infty) \to \B(\infty) \cup \{0\}$ \cite{Jan}*{10.12}. The 
operators $\tilde E_i$ are locally nilpotent, so it makes sense to set 
$e_{\alpha_i} = e_i(b) = \max \{k \in \NN \mid \tilde E_i^k(b) \neq 0\}$ for 
each $b \in \B$. 
...
It follows easily from \cite{Jan}*{4.9 (4)} that $\Delta(F_i^{(r)}) = 
\sum_{j=0}^r v_i^{j(r-j)} F^{(j)}_i \ot F^{(r-j)}_i K_i^{-j}$. Since the 
canonical basis is contained in $U_\A$, it follows that $\Delta(G(b''))$ is a 
sum of terms of the form $p (G(b_{(1)}) \ot G(b_{(2)}) K_\mu)$ with $p \in \A, 
b_{(1)}, b_{(2)} \in \B(\infty)$, and $\mu$ the weight of $b_{(1)}$. Among all 
these terms there is one of the form $d_{b,b'}^{b''} (G(b) \ot G(b') 
K_{wt(b)})$, perhaps with $d_{b,b'}^{b''} = 0$, and from the reasoning above 
we get that $c_{b,b'}^{b''} = v^{(wt(b), \lambda')} d_{b,b'}^{b''}$. Notice 
that unlike before, the element $d_{b,b'}^{b''}$ is independent of $\lambda$ 
and $\lambda'$.
...
of $W$. For each $s$ in the image of $\Lambda_{\tilde w_0}$ we write $b_s = 
\Lambda_{\tilde w_0}^{-1}(s)$, and $F^{(s)} = F_{i_1}^{s_1} \cdots 
F_{i_N}^{s_N}$. If $b_s \in \B(\infty)_{-\nu}$ then for each $s' \in 
\Lambda_{\tilde w_0} (\B(\infty)_{-\nu})$ with $s' >_{\lex} s$ there exist 
$x_{s,s'}, y_{s,s'} \in \A$ such that
\begin{align*}
F^{(s)}
  &= G(b_s) + \sum_{s' >_{\lex} s} x_{s,s'} G(b_{s'})\\
G(b_s) 
  &= F^{(s)} + \sum_{s' >_{\lex} s} y_{s,s'} F^{(s')}
\end{align*}
\end{Lemma*}
\begin{proof}
...
if $b_s v_{\lambda} \neq 0$ then there exis $x_{s,s'}$ as in the statement
such that
\begin{align*}
F^{(s)}v_\lambda
  &= G(b_s)v_\lambda + \sum_{s' >_{\lex} s} x_{s,s'} G(b_{s'})v_\lambda.
\end{align*}
...
since there is only a finite number of elements $s' >_{\lex} s$ such that 
$b_{s'} \in \B(\infty)_{-\nu}$.
\end{proof}
...
\end{Proposition*}
\begin{proof}
We have already observed that $\Delta(F_i^{(r)}) = \sum_{j+k = r} v_i^{jk} 
F_i^{(j)} \ot F_i^{(k)} K^{-j}_i$. A simple induction shows that for each
$s \in \NN^N$ we get $\Delta(F^{(s)}) = \sum_{t+u = s} v^{z(t,u)} F^{(t)}
\ot F^{(u)}K_{wt(F^{(t)})}$. Now the previous lemma implies that
\begin{align*}
\Delta(G(b_s))
  &= \Delta(F^{(s)}) + \sum_{s' >_{\lex}} y_{s,s'} \Delta(F^{s'}) \\
  &= \sum_{t+u = s} v^{z(t,u)} F^{(t)} \ot F^{(u)}K_{wt(F^{(t)})} 
  + \sum_{s' >_{\lex} s} \sum_{t+u = s'} y_{s,s'} v^{z(t,u)} F^{(t)}
    \ot F^{(u)}K_{wt(F^{(t)})}\\
  &= \sum_{t+u = s} v^{z(t,u)} G(b_t) \ot G(b_u) K_{wt(b_t)}
    + \sum_{t+u >_{\lex} s} y'_{t,u} G(b_t) \ot G(b_u) K_{wt(b_t)},
\end{align*}
...
\end{Theorem}
\begin{proof}
For each dominant weight $\lambda$ and each $s \in \Lambda_{\tilde w_0}
(\B(\lambda))$, we denote by $b_{s,\lambda}$ the matrix coefficient in 
$C^+(\lambda)$ correspoding to the only element $b \in \B(\lambda)$ such that 
$\Lambda_{\tilde w_0}(b) = s$. 

It follows from \ref{coproduct-coefficients} that for each pair of dominant 
weights $\lambda, \lambda'$ and each $s \in \Lambda_{\tilde w_0}(\B(\lambda)), 
s' \in \Lambda_{\tilde w_0}(\B(\lambda')), s'' \in 
\Lambda_{\tilde w_0}(\B(\lambda + \lambda'))$ such that $s'' \geq_{\lex} 
s + s'$, there exist $x_{s''} \in \A$ such that 
\begin{align*}
b_{s,\lambda} b_{s',\lambda'} = \sum_{s'' \geq s+s'} x_{s''} 
  b_{s'',\lambda + \lambda'},
\end{align*}
with $x_{s+s'}$ a power of $v$. This shows that the bases defined in
\ref{Littelman-parametrizations} are $(\tilde S^I_{\tilde w_0, \tilde w}, 
\leq_{\lex})$ bases.
