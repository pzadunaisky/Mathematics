...
\cite{C}*{Theorem 2.3}. We give a sketch of the proof in the following paragraphs.
...
Fix $b \in \B(\infty)_{-\nu}$ with $e = e_i(b)$; by \cite{Jan}*{10.15} we have 
$e = \max \{k \in \NN \mid b \in \tilde F_i^k \B(\infty)\}$. We claim that 
$e = \max \{k \mid L_i^{(k)}G(b)^* \neq 0\}$. Suppose that $t > e$. Then by 
definition $L_i^{(t)}(G(b)^*)$ lies in $U^+_{\nu - t \alpha_i}$. Let $b' \in 
\B(\infty)_{-\nu + t \alpha_i}$, so $(G(b'), L_i^{(t)} G(b)^*) = (F^{(t)} 
G(b'), G(b)^*)$. Now $F^{(t)}(U^-) = \tilde F^t(U^-)$ by 
\cite{Jan}*{Lemma 11.3 (2)}, and since $b \notin \tilde F^t(U^-)$ we get that 
$(G(b'), L^{(i)} G(b)^*) = 0$ for all $b' \in 
\B(\infty)_{-\nu + t \alpha_i}$, which implies $L_i^{(t)}G(b)^* = 0$.
