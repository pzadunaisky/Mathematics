...
\sum_{i+j=r}^r v_\alpha^{ji} F^{(i)}_\alpha \ot F^{(j)}_\alpha K_\alpha^{-j}$,
...
\cite{C}*{Theorem 2.3}. We give a shorter proof in the following paragraph.
...
G(b) 
	...
	\mod M_{e+1}
\end{align}
Now applying \cite{Jan}*{Lemma 11.3} repeatedly we get
\begin{align*}
F_\alpha^{(r)} G(b) 
...
which states that given $\lambda$ be a dominant weight and $v_\lambda \in 
V_\A(\lambda)$, then if $b^a v_{\lambda} \neq 0$ there exist $x_{a,a'}$ as in 
the statement such that
\begin{align*}
F^{(a)}v_\lambda
...
v_\alpha^{ij} F_\alpha^{(i)} \ot F_\alpha^{(j)} K^{-j}_\alpha$, and it follows 
