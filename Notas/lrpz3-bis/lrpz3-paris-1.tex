...
$\tilde E_i, \tilde F_i: \B \to \B \cup \{0\}$. These in turn induce the 
\emph{Kashiwara operators} $\tilde e_i, \tilde f_i: G(\B) \to G(\B) \cup 
\{0\}$. The operators $\tilde E_i$ are locally nilpotent, and hence so are the 
$\tilde e_i$, so it makes sense to set $\varepsilon_i(b) = \max \{k \in \NN 
\mid \tilde e_i(b) \neq 0\}$ for each $b \in \B$. Write $\overline e_i(b)$ for 
$\tilde e_i^{\varepsilon_i(b)}(b)$.
