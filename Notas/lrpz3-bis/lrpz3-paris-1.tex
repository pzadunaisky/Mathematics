...
K_\lambda)$, perhaps with $d_{b,b'}^{b''} = 0$, and from the reasoning above 
we get that $c_{b,b'}^{b''} = q^{(\lambda, \lambda')} d_{b,b'}^{b''}$. Notice 
...
\begin{Lemma}
\begin{enumerate}
\item For each $i = 1, \ldots, n$ and each $b \in \B(\infty)$ we get $F_i^{(r)}
G(b) = \qbinom{r+ e_i(b)}{r} G(\tilde F_i^r b) + \sum_{e_i(b') > r + e_i(b)} 
a_{b'} G(b')$.
...
\end{align*}
for some $u \in U_\A^-$. Now as stated in the proof of 
\cite{Jan}*{Lemma 11.12}, page 249, $\tilde G^\alpha(b') = 
G(\tilde F_i^r \tilde E_i^r b') + \tilde F_i^{r+1} u'$ for some 
$u' \in U_\A^-$ whenever $e_i(b') > 0$; notice that $G^\alpha(b') = 
G(b')$ by \cite{Jan}*{11.15}. Taking $b' = \tilde F_i^r b$, we get
$\tilde F_i^r G(b) = G(\tilde F_i^r b) - \tilde F_i^{r+1}u'$, so
