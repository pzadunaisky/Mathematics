...
\color{red}
\begin{Example*}
The following example will follow us throughout the rest of this article.
...
A lattice is said to be \emph{finite} if its underlying set is finite, and 
\emph{distributive} if the binary operation $\meet$ is distributive over 
$\join$, and viceversa. An element $z$ of a distributive lattice is said to be 
\emph{join-irreducible} if it is not the minimum, and whenever $z = x \join y$ 
then $z = x$ or $z = y$.  

Let $\L$ be a distributive lattice, and let $J(\L) = \{x_1, \ldots, x_r\}$ be 
the set of all join irreducible elements. Set $\str(\L)$ to be the subset
...
We will give a presentation of $\str(\L)$ as a quotient of $F(\L)$.

Recall that $\{x_1, \ldots, x_r\}$ is the set of join-irreducible elements
of $\L$. We denote by $[l]$ the image of an element $l \in \L$ in $F(\L)$ and
define a map $\pi: F(\L) \to \str(\L)$ given by $\pi([l]) = e_0 + \sum_{x_i 
\leq l} e_i$. As shown in \cite{RZ2}*{Proposition 3.3.3} this map is 
surjective. Furthermore $L(\pi)$ is the equivalence relation generated by
the set $\{([l] + [l'], [l \join l'] + [l \meet l']) \mid l \in \L\}$. Notice 
also that the Hilbert basis of $\str(\L)$ is precisely the image of $\L$. 
...
\begin{Example*}
Let $u,v \in \NN$ with $u \leq v$, and denote by $\Pi_{u,v}$ the subset of
$\NN^u$ consisting of elements $(i_1, ldots, i_u)$ such that $1 \leq i_1 <
\cdots < i_u \leq v$ endowed with the restriction of the natural product
order on $\NN^u$. This is a distributive sublattice of $\NN^u$.
\end{Example*}
