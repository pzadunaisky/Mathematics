...
$G$-graded $A$-module, i.e. they are $\Hom_A^H(-,\phi_!(A))$-acyclic and hence 
...
\bigskip
Throughout this document $\k$ is a commutative ring, and unadorned $\hom$ 
...
\label{COG-INJDIM}
Throughout this section $G$ and $H$ are groups, $\phi: G \to H$ denotes a 
group morphism and we set $L = \ker \phi$. Also $A$ denotes a $G$-graded 
$\k$-algebra.
...
be the morphism $\psi(z_1, \ldots, z_r) = z_1 + \cdots + z_r$. Then $\psi_!(A)$is $\NN$-graded, so the previous theorem implies that $\injdim_A^{\ZZ^r} A = 
